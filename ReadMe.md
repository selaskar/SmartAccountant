...
* All `ValidationExceptions`, `ServerExceptions` and derivatives of `EnumException` (e.g., `ImportException`) are handled by their respective exception filters.
...
* Add the following to the beginning of EF migration classes to supress code style and quality checks:
...
* All date/times in database are stored in UTC time zone.
...
* Name test methods as they have 'should'/'must' in the beginning of the name.
